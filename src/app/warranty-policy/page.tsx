...
export default function WarrantyPolicy() {
	const policys: IContent[] = [
		...
		},
		{
			...
			contents: [
				{
					category: '2.1. Tư vấn hỗ trợ kiểm tra sản phẩm trước khi mua',
					type:'second',
	...
	const Content = ( props: IContent) => {
		...
		return (
			<div className='content'>
				...
				<ul className={type =='third' || type=='second' ? 'listTypeDot': ''}>
					{
						contents.map((ctn, i) => {
							if(typeof ctn === 'object' && ctn !== null){
								return <Content category={ctn.category} contents={ctn.contents} type={ctn?.type} ></Content>
							}
				...
				<div className="mainContent grid wide">
					<div className='title l-8'>
						...
						<p>
							...
							Thinkmay cam kết không ngừng tự cải thiện và nâng cao chất lượng phục vụ.
						</p>
					</div>
					...
					<div className="wrapperPolicys">
						{
							policys.map(plc => (
								<Content category={plc.category} contents={plc.contents} type={plc.type}></Content>
							))
						}
					</div>
				</div>
